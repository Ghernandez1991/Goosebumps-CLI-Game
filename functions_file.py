...
def petting_zoo() -> None:
    ...
    time.sleep(2)
    print(
        "you are presented with a choice. Turn right for the domestic animals, turn left for wild animals."
    )
    ...
    while True:
        ...
        player_direction_choice = input(
            "which do you choose? Press enter when complete"
        )
    ...
    if player_direction_choice in left_choices:
        print(
            f"You have chosen to take your chances with the wild animals and go {player_direction_choice}"
        )
        playsound(r"audio\elephant_edit.mp3", False)

    else:
        print(
            f"You have elected to take your chances with the domesticated animals and go {player_direction_choice}"
        )
...
def house_of_mirrors_gameboard() -> None:
    ...
    while True:
        ...
        punjee_pit_starting_position = punjee_pit_starting_position_xaxis + str(
            punjee_pit_starting_position_yaxis
        )
        ...
        breakout_room_starting_position = breakout_room_starting_position_xaxis + str(
            breakout_room_starting_position_yaxis
        )
    ...
    print(
        f"The start row index is {indexed_start_y_point}, the start column index is{indexed_start_x_point}"
    )
    ...
    print(
        f"The exit row index is {indexed_exit_y_point}, the exit column index is{indexed_exit_x_point}"
    )
    ...
    print(
        f"The start row index for punjee is {indexed_punjee_start_y_point}, the start column index for punjee is{indexed__punjee_start_x_point}"
    )
    # using the row and column index values, place 'exit_point' onto our game board(csv file)
    ...
    print(
        f"The start row index for punjee is {indexed_breakout_start_y_point}, the start column index for punjee is{indexed_breakout_start_x_point}"
    )
    # using the row and column index values, place 'exit_point' onto our game board(csv file)
    df.iat[
        indexed_breakout_start_y_point, indexed_breakout_start_x_point
    ] = "breakout_room"
...
def move_function(starting_position):
    ...
    if column in columns:
        ...
        print(
            f" The row value {column} is in the {value_of_column} postion in the list columns"
        )
    else:
        print("item not in list")
...
def forward_function(value_of_column, value_of_row):
    ...
    try:
        ...
        current_position_count = np.column_stack(
            [df[col].str.contains("current_position", na=False) for col in df]
        )
        ...
        if current_position_count.any() > 0:
            ...
            global current_position
            current_position = new_forward_column_position + str(
                new_forward_row_position
            )
            ...
            df.iat[
                new_forward_row_position, new_forward_column_position
            ] = "current_position"
            print(
                f"the new forward row position is {new_forward_row_position}, the new_forward_column_position is {new_forward_column_position}  "
            )
            print(df)
...
def backward_function(value_of_column, value_of_row):
    ...
    try:
        ...
        if current_position_count.any() > 0:
            ...
            global current_position
            current_position = new_backward_column_position + str(
                new_backward_row_position
            )
            ...
            # take the index value and place 'current position' in the spreadsheet
            df.iat[
                new_backward_row_position, new_backward_column_position
            ] = "current_position"
            print(
                f"the new backward row position is {new_backward_row_position}, the new_backward_column_position is {new_backward_column_position}  "
            )
            print(df)
            ...
            print(
                f"the new forward row position is {new_backward_row_position}, the new_forward_column_position is {new_backward_column_position}  "
            )
            print(df)
...
def left_function(value_of_column, value_of_row):
    ...
    try:
        ...
        if current_position_count.any() > 0:
            ...
            print(
                f"the new left row position is {new_left_row_position}, the new_left_column_position is {new_left_column_position}  "
            )
            print(df)
...
def right_function(value_of_column, value_of_row):
    ...
    try:
        ...
        if current_position_count.any() > 0:
            ...
            # take the index value and place 'current position' in the spreadsheet
            df.iat[
                new_right_row_position, new_right_column_position
            ] = "current_position"
            print(
                f"the new right row position is {new_right_row_position}, the new_right_column_position is {new_right_column_position}  "
            )
            print(df)
...
def breakout_room() -> None:
    ...
    print(
        f"Well {player_input}, you have to correctly respond to my commands, or you will trapped for ever. You have {strikes} strikes until the doors lock forever"
    )
    ...
    for item in list_of_words:
        ...
        if user_input == item:
            ...
            if correct_guesses == len(list_of_words):
                t.cancel()
                print(
                    "You successfully printed all the words. Please make another selection to escape the house of mirrors"
                )
...
def player_movement_choice() -> None:
    while current_position != exit_position:
        player_direction = int(
            input(
                "Which direction do you want to go 1:Forward 2:Backrward 3:Left 4:Right"
            )
        )
...
def house_of_mirrors() -> None:
    ...
    if direction_choice in left_choices:
        ...
        img.show()
        print(
            "You must make your selections carefully, or else you will trapped forever."
        )
        ...
        print(
            "You decided to turn right and have thus avoided the prospect of being trapped forever"
        )
